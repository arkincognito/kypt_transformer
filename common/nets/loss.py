    ...
    def forward(self, joints_gt, joint_valid_in, joint_loc_2d_gt, cam_param_pred, rel_trans_valid, rel_trans_pred):
        ...
        with torch.no_grad():
            ...
            for i in range(1):
                ...
                tmp = torch.inverse(
                    torch.matmul(mat_A.transpose(2, 1), mat_A)
                    + torch.eye(3, device=mat_A.device, dtype=mat_A.dtype) * 1e-4
                )
                cam_param_gt_curr = (
                    torch.matmul(tmp, torch.matmul(mat_A.transpose(2, 1), b.unsqueeze(2))).squeeze(2).unsqueeze(0)
                )  # 1 x N x 3
...
class HandTypeLoss(nn.Module):
    ...
    def forward(self, hand_type_out, hand_type_gt, hand_type_valid):
        ...
        hand_type_gt_val = (
            hand_type_gt[:, 0] + hand_type_gt[:, 1] * 2 - 1
        ) * hand_type_valid  # 0 - right, 1 - left, 2 - both
...
class PoseLoss(nn.Module):
    ...
    def forward(self, pose_out, pose_gt, mano_valid):
        ...
        normalizer = 0  # 16 * mano_valid[0] + 16 * mano_valid[1]
        start_indx = 0

        if cfg.hand_type in ["right", "both"]:
            loss_right = (pose_out[:, :, :16] - pose_gt[:, :16].unsqueeze(0)) * mano_valid[:, :1].unsqueeze(
                2
            ).unsqueeze(0)
        ...
        if cfg.hand_type in ["left", "both"]:
            loss_left = (pose_out[:, :, start_indx:] - pose_gt[:, 16:].unsqueeze(0)) * mano_valid[:, 1:].unsqueeze(
                2
            ).unsqueeze(0)
...
class ShapeLoss(nn.Module):
    ...
    def forward(self, shape_gt, shape_pred, mano_valid):
        ...
        shape_gt1 = (shape_gt[:, :10] * mano_valid[:, :1] + shape_gt[:, 10:] * mano_valid[:, 1:]) / (
            torch.sum(mano_valid, 1, keepdim=True) + 1e-8
        )
...
class ObjPoseLoss(nn.Module):
    ...
    def forward(
        ...
        obj_bb_gt = torch.matmul(obj_bb_rest, batch_rodrigues(obj_rot_gt).transpose(2, 1)) + obj_trans_gt.unsqueeze(
            1
        )  # N x 8 x 3
        ...
        flipped_z_obj_rot = torch.matmul(
            batch_rodrigues(rot_dir_z), batch_rodrigues(obj_rot_gt)
        )  # N x 3 x 3 # flipped rot
        obj_bb_flipped_z_gt = torch.matmul(obj_bb_rest, flipped_z_obj_rot.transpose(2, 1)) + obj_trans_gt.unsqueeze(
            1
        )  # N x 8 x 3

        # rotation about y
        rot_dir_y = batch_rodrigues(obj_rot_gt)[:, :3, 1] * rot_angle  # N x 3
        flipped_y_obj_rot = torch.matmul(
            batch_rodrigues(rot_dir_y), batch_rodrigues(obj_rot_gt)
        )  # N x 3 x 3 # flipped rot
        obj_bb_flipped_y_gt = torch.matmul(obj_bb_rest, flipped_y_obj_rot.transpose(2, 1)) + obj_trans_gt.unsqueeze(
            1
        )  # N x 8 x 3
        ...
        obj_bb_flipped_yz_gt = torch.matmul(obj_bb_rest, flipped_yz_obj_rot.transpose(2, 1)) + obj_trans_gt.unsqueeze(
            1
        )  # N x 8 x 3

        # get flipped z bb
        is_z_rot_sym_objs = torch.logical_or(
            obj_id == 6, obj_id == 21
        )  # N # ['006_mustard_bottle', '021_bleach_cleanser']
        ...
        obj_bb_flipped_z_gt = obj_bb_flipped_z_gt * is_z_rot_sym_objs[:, None, None] + obj_bb_gt * (
            torch.logical_not(is_z_rot_sym_objs[:, None, None])
        )

        # get flipped y and yz bb
        is_yz_rot_sym_objs = torch.zeros_like(obj_id) * 1.0
        obj_bb_flipped_y_gt = obj_bb_flipped_y_gt * is_yz_rot_sym_objs[:, None, None] + obj_bb_gt * (
            torch.logical_not(is_yz_rot_sym_objs[:, None, None])
        )
        obj_bb_flipped_yz_gt = obj_bb_flipped_yz_gt * is_yz_rot_sym_objs[:, None, None] + obj_bb_gt * (
            torch.logical_not(is_yz_rot_sym_objs[:, None, None])
        )
        ...
        if obj_corner_pred is not None:
            obj_corner_pred1 = obj_corner_pred.reshape(
                obj_corner_pred.shape[0], obj_corner_pred.shape[1], 8, 2
            )  # 6 x N x 8 x 2
            loss_obj_corner_pred = torch.abs(
                obj_corner_pred1 - obj_corner_gt[:, :, :2].unsqueeze(0)
            ) * obj_pose_valid_bowl.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        else:
        ...
        if cfg.use_obj_rot_parameterization:
            ...
            obj_bb_pred = torch.matmul(obj_bb_rest1, rot_mat.transpose(2, 1)) + obj_trans_pred.reshape(-1, 3).unsqueeze(
                1
            )  # 6*N x 8 x 3
            loss_obj_rot = torch.zeros((1,)).to(obj_rot_pred.device)
        else:
            obj_bb_pred = torch.matmul(
                obj_bb_rest1, batch_rodrigues(obj_rot_pred.reshape(-1, 3)).transpose(2, 1)
            ) + obj_trans_pred.reshape(-1, 3).unsqueeze(
                1
            )  # 6*N x 8 x 3
            loss_obj_rot = torch.abs(obj_rot_pred - obj_rot_gt.unsqueeze(0)) * obj_pose_valid_bowl.unsqueeze(
                0
            ).unsqueeze(2)

        # 2d projection error
        obj_bb_2d_pred = (
            obj_bb_pred[:, :, :2].reshape(cam_param_pred.shape[0], cam_param_pred.shape[1], 8, 2)
            * cam_param_pred[:, :, None, :1]
            + cam_param_pred[:, :, None, 1:]
        )  # 6 x N x 8 x 2
        ...
        loss_corners_unflipped = (
            torch.abs(obj_bb_pred1 - obj_bb_gt.unsqueeze(0)) * 1000 * obj_pose_valid1
        )  # 6 x N x 8 x 3
        loss_corners_unflipped = torch.mean(torch.mean(loss_corners_unflipped, dim=3), dim=2)

        loss_corners_flipped_z = (
            torch.abs(obj_bb_pred1 - obj_bb_flipped_z_gt.unsqueeze(0)) * 1000 * obj_pose_valid1
        )  # 6*N
        loss_corners_flipped_z = torch.mean(torch.mean(loss_corners_flipped_z, dim=3), dim=2)

        loss_corners_flipped_y = (
            torch.abs(obj_bb_pred1 - obj_bb_flipped_y_gt.unsqueeze(0)) * 1000 * obj_pose_valid1
        )  # 6*N
        loss_corners_flipped_y = torch.mean(torch.mean(loss_corners_flipped_y, dim=3), dim=2)

        loss_corners_flipped_yz = (
            torch.abs(obj_bb_pred1 - obj_bb_flipped_yz_gt.unsqueeze(0)) * 1000 * obj_pose_valid1
        )  # 6*N
        ...
        loss_obj_trans = torch.abs(obj_trans_pred - obj_trans_gt.unsqueeze(0)) * obj_pose_valid_bowl.unsqueeze(
            0
        ).unsqueeze(2)
        ...
        if obj_trans_left_pred is not None:
            ...
            loss_obj_trans_left = torch.abs(
                obj_trans_left_pred - obj_trans_left_gt.unsqueeze(0)
            ) * obj_pose_valid_bowl.unsqueeze(0).unsqueeze(2)
            loss_obj_hand_tri = torch.abs(
                hand_rel_trans_pred + obj_trans_left_pred - obj_trans_pred
            ) * obj_pose_valid_bowl.unsqueeze(0).unsqueeze(2)
...
class ManoMesh:
    ...
    def __init__(self):
        ...
        if (
            torch.sum(
                torch.abs(self.mano_layer["left"].shapedirs[:, 0, :] - self.mano_layer["right"].shapedirs[:, 0, :])
            )
            < 1
        ):
        ...
        self.jointsMapManoToDefault = [16, 15, 14, 13, 17, 3, 2, 1, 18, 6, 5, 4, 19, 12, 11, 10, 20, 9, 8, 7, 0]
    ...
    def get_mano_mesh(self, pose, shape, rel_trans_in, rel_trans_valid, cam_param):
        ...
        if cfg.hand_type in ["right", "both"]:
            mesh_right = self.mano_layer["right"](
                global_orient=pose[:, 0, :3].to(torch.device("cpu")),
                hand_pose=pose[:, 1 : cfg.num_joint_queries_per_hand]
                .reshape(pose.shape[0], -1)
                .to(torch.device("cpu")),
            ...
            joints_pred["joints_mano_right"] = (
                joints_pred["joints_mano_right"].view(cfg.dec_layers, -1, 21, 3).to(pose.device)
            )  # 6 x N x 21 x 3

            joints_pred["joints_right"] = (
                torch.matmul(self.ih26m_joint_regressor, joints_pred["mesh_right"])
                .view(cfg.dec_layers, -1, 21, 3)
                .to(pose.device)
            )  # 6 x N x 21 x 3
            joints_pred["mesh_right"] = (
                joints_pred["mesh_right"]
                .view(cfg.dec_layers, -1, joints_pred["mesh_right"].shape[1], 3)
                .to(pose.device)
            )  # 6 x N x 778 x 3
            joints_pred["joints2d_right"] = joints_pred["joints_right"][:, :, :, :2] * cam_param[:, :, :1].unsqueeze(
                2
            ) + cam_param[:, :, 1:].unsqueeze(2)
        ...
        if cfg.hand_type in ["left", "both"]:
            mesh_left = self.mano_layer["left"](
                global_orient=pose[:, start_indx, :3].to(torch.device("cpu")),
                hand_pose=pose[:, (start_indx + 1) :].reshape(pose.shape[0], -1).to(torch.device("cpu")),
                betas=shape.to(torch.device("cpu")),
                transl=rel_trans.to(torch.device("cpu")),
            )  # 6N x 778 x 3
            ...
            joints_pred["joints_mano_left"] = (
                joints_pred["joints_mano_left"].view(cfg.dec_layers, -1, 21, 3).to(pose.device)
            )  # 6 x N x 21 x 3

            joints_pred["joints_left"] = (
                torch.matmul(self.ih26m_joint_regressor, joints_pred["mesh_left"])
                .view(cfg.dec_layers, -1, 21, 3)
                .to(pose.device)
            )  # 6 x N x 21 x 3
            joints_pred["mesh_left"] = (
                joints_pred["mesh_left"].view(cfg.dec_layers, -1, joints_pred["mesh_left"].shape[1], 3).to(pose.device)
            )  # 6 x N x 778 x 3
...
class Joints2p5dLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.joint_recon_order = [3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12, 19, 18, 17, 16]
    ...
    def forward(self, joints_2p5d_pred, joints_coord_gt, joints_valid):
        ...
        parent_inds = (
            self.parent_inds.unsqueeze(1).unsqueeze(0).repeat(1, 1, 3).repeat(bs, 1, 1).to(joints_2p5d_pred.device)
        )  # N x 42 x 3
        ...
        parent_rel_valid_right = torch.cat(
            [parent_rel_valid_right, torch.zeros(parent_rel_valid_right.shape[0], 1).to(parent_rel_valid_right.device)],
            dim=1,
        )  # N x 21
        ...
        parent_rel_valid_left = torch.cat(
            [parent_rel_valid_left, torch.zeros(parent_rel_valid_left.shape[0], 1).to(parent_rel_valid_left.device)],
            dim=1,
        )  # N x 21
...
class JointVectorsLoss(nn.Module):
    ...
    def forward(self, joint_vec_preds_in, joints_gt_in, joint_valid_in, joint_loc_2d_gt, cam_param, rel_trans_pred):
        ...
        parent_inds = (
            self.parent_inds.unsqueeze(1).unsqueeze(0).repeat(1, 1, 3).repeat(bs, 1, 1).to(joints_gt.device)
        )  # N x 42 x 3

        joint_vec_gt_right = joints_gt[:, :20] - torch.gather(joints_gt[:, :21], 1, parent_inds[:, :20])  # N x 20 x 3
        joint_vec_gt_left = joints_gt[:, 21:41] - torch.gather(
            joints_gt[:, 21:], 1, parent_inds[:, 21:41] - 21
        )  # N x 20 x 3
        ...
        loss_joint_vec = torch.abs(joint_vec_preds * 1000 - joint_vec_gt.unsqueeze(0)) * joint_vec_valid.unsqueeze(
            0
        ).unsqueeze(3)
        ...
        for j in range(5):
            for i in range(4):
                if cfg.hand_type in ["both", "right"]:
                    if i == 0:
                        joint_3d_right = joint_vec_preds[:, :, self.joint_recon_order[j * 4 + i]] + 0.0
                    else:
                ...
                if cfg.hand_type in ["both", "left"]:
                    ...
                    if i == 0:
                        joint_3d_left = joint_vec_preds[:, :, self.joint_recon_order[j * 4 + i] + offset] + 0.0
                    else:
                        joint_3d_left = (
                            joint_vec_preds[:, :, self.joint_recon_order[j * 4 + i] + offset] + joint_list_left[-1]
                        )
        ...
        if cfg.hand_type == "both":
            ...
            joint_3d_valid = torch.cat([joint_3d_valid_right, joint_3d_valid_left], dim=1)  # N x 40
        ...
        loss_joint_3d = torch.abs(joint_3d_preds * 1000 - joint_3d_gt.unsqueeze(0)) * joint_3d_valid.unsqueeze(
            0
        ).unsqueeze(3)
        ...
        if cfg.use_2D_loss:
            joints_2d_pred = (joint_3d_preds + trans)[:, :, :, :2] * cam_param[:, :, :1].unsqueeze(2) + cam_param[
                :, :, 1:
            ].unsqueeze(2)
            loss_joint_2d = torch.abs(joints_2d_pred - joint_2d_gt.unsqueeze(0)) * joint_3d_valid.unsqueeze(
                0
            ).unsqueeze(3)
        else:
        ...
        return (
            ...
            loss_joint_3d,
            loss_joint_2d,
            joint_3d_preds_orig_order_right,
            joint_3d_preds_orig_order_left,
        )
...
class JointLoss(nn.Module):
    ...
    def forward(
        self, rel_trans_in, joints_gt, joint_valid, joint_loc_2d_gt, rel_trans_valid, joints_pred, cam_param=None
    ):
...
class VertexLoss(nn.Module):
    ...
    def forward(self, verts_gt, mano_valid, joints_pred):
        ...
        if cfg.hand_type == "both":
            ...
            verts_valid = torch.ones((bs, 778, 3), device=mesh_right.device) * mano_valid[:, :1].unsqueeze(2).unsqueeze(
                0
            )
...
class JointClassificationLoss(nn.Module):
    ...
    def forward(
        ...
        gt_inds_batch, row_inds_batch_list, asso_inds_batch_list = nearest_match_2djoints(
            joint_loc_gt_np,
            joint_loc_pred_np,
            joint_valid_np,
            mask,
            joint_class_pred,
            joint_3d_gt.detach().cpu().numpy(),
            peak_joints_map_batch_np,
            target_obj_kps_heatmap_np,
            obj_kps_coord_gt_np,
            obj_kps_3d_gt_np,
        )
        ...
        joint_class_pred1 = joint_class_pred.reshape(-1, joint_class_pred.shape[-1]).masked_select(
            mask1.reshape(-1).unsqueeze(1)
        )

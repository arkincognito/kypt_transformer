...
class InterHand26M(torch.utils.data.Dataset):
    def __init__(self, transform, mode, annot_subset="all", capture_test=None, camera_test=None, seq_name_test=None):
        ...
        if cfg.trans_test == "rootnet":
            if self.annot_subset == "machine_annot" and self.mode == "val":
                self.rootnet_output_path = os.path.join(
                    cfg.root_net_output_path, "rootnet_interhand2.6m_output_machine_annot_val.json"
                )
            else:
                self.rootnet_output_path = os.path.join(
                    cfg.root_net_output_path, "rootnet_interhand2.6m_output_all_test.json"
                )
        ...
        for aid in tqdm(list(db.anns.keys())[::slice_interval]):  ## change slice interval to make the test brief
            ...
            if str(frame_idx) not in mano_params[str(capture_id)].keys():
                print(
                    "Frame %s in capture Id %s does not have MANO model (skipping)..."
                    % (str(frame_idx), str(capture_id))
                )
                continue
            ...
            for ii, ht in enumerate(mano_hand_type):
                ...
                if mano_param is not None and np.sum(joint_valid[self.joint_type[ht]]) > 0:
                    pose, trans, shape = transformManoParamsToCam(
                        np.array(mano_param["pose"]),
            ...
            if True:
                ...
                if cfg.hand_type == "right":
                    ...
                    if mano_valid[1] == 0 or mano_valid[0] == 1:
                        continue
                else:  # cfg.hand_type == "both"
                    if not self.mode == "test":
                        if hand_type == "interacting" and cfg.predict_type == "angles":
                            if mano_valid[0] == 0:
                                if (
                                    np.sum(joint_valid[self.joint_type["right"]]) != 0
                                    and np.sum(joint_valid[self.joint_type["left"]]) != 0
                                ):
            ...
            if (self.mode == "val" or self.mode == "test") and cfg.trans_test == "rootnet":
                ...
                abs_depth = {
                    "right": rootnet_result[str(aid)]["abs_depth"][0],
    ...
    def get_mano_vertices(self, pose, shape, transl, hand_type):
        mesh = self.mano_layer[hand_type](
    ...
    def __getitem__(self, idx):
        ...
        mano_pose, mano_trans, mano_shape, mano_valid = (
            data["mano_pose"].copy(),
            data["mano_trans"].copy(),
            data["mano_shape"].copy(),
            data["mano_valid"].copy(),
        )
        ...
        if cfg.predict_type == "vectors":
            root_valid = (
                np.array([joint_valid[self.root_joint_idx["right"]] * joint_valid[self.root_joint_idx["left"]]]) * 1.0
            )
        ...
        if cfg.predict_type == "angles":
            ...
            if root_valid and (np.sum(joint_valid[:21]) == 0):  # when its only left hand image
                ...
                rel_trans_hands_rTol = np.array([0.2, 0.0, 0.0]).astype(np.float32)
            ...
            if self.mode == "train":
                joint_cam_no_trans = self.restore_coord_cam_from_img(
                    joint_coord[:42].copy(), inv_trans_no_rot, data, do_flip
                )
            else:
                joint_cam_no_trans = joint_cam.copy().astype(np.float32)

            rel_trans_hands_rTol = (
                joint_cam_no_trans[self.root_joint_idx["left"]] - joint_cam_no_trans[self.root_joint_idx["right"]]
            ) / 1000
        ...
        if cfg.predict_type == "angles" and self.mode == "test":
            ...
            verts_left, faces_left = self.get_mano_vertices(
                mano_pose[48:], mano_shape[10:], rel_trans_hands_rTol, "left"
            )  # add the rel trans in loss.py
        ...
        if (
            (np.sum(hand_type) == 2 and np.sum(joint_valid[:42]) != 42)
            or ((hand_type[0] == 1 and hand_type[1] == 0) and np.sum(joint_valid[:21]) != 21)
            or ((hand_type[0] == 0 and hand_type[1] == 1) and np.sum(joint_valid[21:]) != 21)
        ):
            hm_valid = np.array([0.0]).astype(np.float32)
        else:
            hm_valid = np.array([1.0]).astype(np.float32)

        inputs = {"img": img, "mask": mask}

        targets = {
            "joint_coord": joint_coord[:42],
            "rel_trans_hands_rTol": rel_trans_hands_rTol,
            "hand_type": hand_type,
            "mano_pose": mano_pose,
            "mano_shape": mano_shape,
            "joint_cam_no_trans": joint_cam_no_trans,
            "verts": verts,
            "contact_tri_verts_inds": contact_verts_gt,
            "contact_tri_inds": contact_tri_inds_gt,
            "contact_vert_coord": interest_vert_coord,
            "contact_vis": contact_vis_np,
        }

        meta_info = {
            "joint_valid": joint_valid[:42],
            ...
            "seq_id": (seq_name[:9]),
            "contact_valid": contact_valid_gt,
            "contact_vert_valid": interset_vert_valid,
            "hm_valid": hm_valid,
    ...
    def dump_results(self, preds, dump_dir):
        for i, frame in enumerate(preds["frame"]):
            ...
            cv2.imwrite(
                osp.join(dump_dir, "patch_" + str(frame) + ".jpg"),
            ...
            if False:
                ...
                for hand_type in ("right", "left"):
                    ...
                    renderer = pyrender.OffscreenRenderer(
                        viewport_width=img_width, viewport_height=img_height, point_size=1.0
                    )
    ...
    def print_results(self, mpjpe_sh, mpjpe_ih, mrrpe, f):
        if len(mrrpe) > 0:
            my_print("MRRPE: %f" % (sum(mrrpe) / len(mrrpe)), f)
    ...
    def evaluate(self, preds, gt, ckpt_path, annot_subset):
        ...
        pred_joints, pred_hand_type_all, pred_rel_trans = (
            preds["joints"],
            preds["hand_type"],
            preds["rel_trans"],
        )
        ...
        for i in range(num_samples):
            ...
            for h in hands_list:
                ...
                gt_joint_coord_cam[self.joint_type[h]] = (
                    gt_joint_coord_cam[self.joint_type[h]] - gt_joint_coord_cam[self.root_joint_idx[h], None, :]
                )

                if cfg.predict_type == "angles":
                    mano_gt_joint_coord_cam[self.joint_type[h]] = (
                        mano_gt_joint_coord_cam[self.joint_type[h]]
                        - mano_gt_joint_coord_cam[self.root_joint_idx[h], None, :]
                    )
                ...
                for j in range(self.joint_num * 2):
                    if joint_valid[j]:
                        ...
                        if np.sum(gt_hand_type) == 1:  # == 'right' or gt_hand_type == 'left':
                            mpjpe_mano_sh[j].append(
                                np.sqrt(np.sum((pred_joint_coord_cam[j] - mano_gt_joint_coord_cam[j]) ** 2))
                            )
                        else:
                            mpjpe_mano_ih[j].append(
                                np.sqrt(np.sum((pred_joint_coord_cam[j] - mano_gt_joint_coord_cam[j]) ** 2))
                            )
            ...
            pickle.dump(
                {
                    "sh": mpjpe_sh,
                    "ih": mpjpe_ih,
                    "fname_sh": frame_name_sh,
                    "fname_ih": frame_name_ih,
                    "sh_mano": mpjpe_mano_sh,
                    "ih_mano": mpjpe_mano_ih,
                    "fname_mano_sh": frame_name_mano_sh,
                    "fname_mano_ih": frame_name_mano_ih,
                },
                f,
            )
    ...
    def evaluate_2p5d(self, preds, gt, ckpt_path, annot_subset):
        ...
        preds_joint_coord, preds_rel_trans, preds_hand_type, inv_trans = (
            preds["joint_coord"],
            preds["rel_trans"],
            preds["hand_type"],
            preds["inv_trans"],
        ...
        for n in range(sample_num):
            ...
            data = {
                "abs_depth": {"left": preds["abs_depth_left"][n], "right": preds["abs_depth_right"][n]},
                "cam_param": {"princpt": gt["princpt"][n], "focal": gt["focal"][n]},
            }

            preds_joint_coord_curr = preds_joint_coord[n].astype(np.float)
            pred_joint_coord_cam = self.restore_coord_cam_from_img(
                preds_joint_coord_curr.copy(), inv_trans[n], data, False
            )
            ...
            for h in ("right", "left"):
                ...
                gt_joint_coord[self.joint_type[h]] = (
                    gt_joint_coord[self.joint_type[h]] - gt_joint_coord[self.root_joint_idx[h], None, :]
                )

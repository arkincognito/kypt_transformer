...
class Model(nn.Module):
    def __init__(self, backbone_net, decoder_net, transformer, position_embedding):
        ...
        if cfg.has_object:
            self.linear_class = MLP(
                cfg.hidden_dim, cfg.hidden_dim, (42 + 1 + 1) if cfg.hand_type == "both" else (21 + 1 + 1), 4
            )
        else:
            self.linear_class = MLP(
                cfg.hidden_dim, cfg.hidden_dim, (42 + 1) if cfg.hand_type == "both" else (21 + 1), 4
            )
        ...
        if cfg.has_object:
            ...
            if cfg.predict_obj_left_hand_trans:
                self.linear_obj_left_rel_trans = MLP(cfg.hidden_dim, cfg.hidden_dim, 3, 3)
    ...
    def render_gaussian_heatmap(self, joint_coord, joint_valid):
        ...
        x = joint_coord1[:, :, 0, None, None]
        y = joint_coord1[:, :, 1, None, None]
        heatmap = (
            torch.exp(-(((xx - x) / cfg.sigma) ** 2) / 2 - (((yy - y) / cfg.sigma) ** 2) / 2)
            * joint_valid1[:, :, None, None]
        )  # N x 42 x h x w
    ...
    def sample_obj_seg(self, obj_seg):
        ...
        peaks, peaks_ind_list = self.obj_peak_detector.detect_peaks_nms(
            obj_seg, cfg.num_obj_samples, intensity_th=15
        )  # , nms_th)
    ...
    def get_input_seq(
        ...
        for ii in range(heatmap_np.shape[0]):
            if use_gt_peak_locs:
                ...
                mask2 = np.logical_or(
                    peaks_ind_list[:, 0] > cfg.output_hm_shape[1] - 1, peaks_ind_list[:, 1] > cfg.output_hm_shape[2] - 1
                )
                ...
                if cfg.has_object:
                    ...
                    if len(obj_peaks_ind_list) > 0:
                        ...
                        mask2 = np.logical_or(
                            obj_peaks_ind_list[:, 0] > cfg.output_hm_shape[1] - 1,
                ...
                peaks, peaks_ind_list = self.peak_detector.detect_peaks_nms(
                    heatmap_np[ii], (cfg.max_num_peaks - cfg.num_obj_samples) if cfg.has_object else cfg.max_num_peaks
                )
            ...
            if len(peaks_ind_list) == 0:
                ...
                if peaks_pixel_locs_normalized.shape[0] < cfg.max_num_peaks:
                    dummy_peaks = np.tile(
                        np.array([[-1, -1]]), (cfg.max_num_peaks - peaks_pixel_locs_normalized.shape[0], 1)
                    )
        ...
        peak_joints_map_batch = torch.from_numpy(np.stack(peak_joints_map_batch, 0)).to(
            pos_embed.device
        )  # N x max_num_peaks
        ...
        positions = nn.functional.grid_sample(
            pos_embed,
            ...
            2
        )  # N x hidden_dim x max_num_peaks

        # Sample the CNN features
        multiscale_features = []
        grids_tensor = (
            torch.from_numpy(np.expand_dims(grids, 1)).float().to(feature_pyramid[cfg.mutliscale_layers[0]].device)
        )
        for layer_name in cfg.mutliscale_layers:
            # N x C x 1 x max_num_peaks
            multiscale_features.append(
                torch.nn.functional.grid_sample(feature_pyramid[layer_name], grids_tensor, align_corners=True)
            )
        ...
        if cfg.has_object:
            ...
            if not cfg.use_big_decoder:
                input_seq_object = self.linear1_obj(
                    multiscale_features[:, :, -(512 + 256) :]
                )  # N x max_num_peaks x hidden_dim
            else:
                input_seq_object = self.linear1_obj(multiscale_features[:, :, -3072:])  # N x max_num_peaks x hidden_dim

            input_seq = input_seq_hands * (
                peak_joints_map_batch.unsqueeze(2) != cfg.obj_cls_index
    ...
    def forward(self, inputs, targets, meta_info, mode, epoch_cnt=1e8):
        ...
        pos = self.position_embedding(
            nn.functional.interpolate(input_img, (cfg.output_hm_shape[2], cfg.output_hm_shape[1])),
            nn.functional.interpolate(input_mask, (cfg.output_hm_shape[2], cfg.output_hm_shape[1])),
        )

        # Get the input tokens
        input_seq, masks, positions, joint_loc_pred_np, mask_np, peak_joints_map_batch = self.get_input_seq(
            joint_heatmap_out,
            obj_seg_out,
            feature_pyramid,
            pos,
        ...
        if cfg.use_bottleneck_hand_type:
            bottleneck_hand_type_feat = F.avg_pool2d(img_feat, (img_feat.shape[2], img_feat.shape[3])).view(
                -1, img_feat.shape[1]
            )
            bottleneck_hand_type = torch.sigmoid(
                self.linear_bottleneck_hand_type(bottleneck_hand_type_feat)
            )  # N x 2 (Right, Left might not be in that order)
        ...
        transformer_out, hand_type, memory, encoder_out, attn_wts_all_layers = self.transformer(
            src=input_seq,
            mask=torch.logical_not(masks),
        ...
        if cfg.hand_type == "both":
            if cfg.predict_type == "angles":
                ## TODO: LeftHand 좌우 반전시켜야하는지 확인!!!!!
                # 반전 없이 그냥 가는 걸 보면...transformer_out 두 번째 그룹이 자동으로 왼손을 인식해야하는 것 같은데?
                ...
                if cfg.predict_2p5d:
                    joint_px_hm = self.linear_joint_2p5d_px(
                        transformer_out[:, : cfg.num_joint_queries_per_hand * 2]
                    )  # 6 x 42 x N x 128
                    joint_py_hm = self.linear_joint_2p5d_py(
                        transformer_out[:, : cfg.num_joint_queries_per_hand * 2]
                    )  # 6 x 42 x N x 128
                    joint_dep_hm = self.linear_joint_2p5d_dep(
                        transformer_out[:, : cfg.num_joint_queries_per_hand * 2]
                    )  # 6 x 42 x N x 128
                    joint_2p5d_hm = torch.cat(
                        [joint_px_hm.unsqueeze(3), joint_py_hm.unsqueeze(3), joint_dep_hm.unsqueeze(3)], dim=3
                    )  # 6 x 42 x N x 3 x 128
                else:
                    joint_vecs = self.linear_joint_vecs(
                        transformer_out[:, : cfg.num_joint_queries_per_hand * 2]
                    )  # 6 x 40 x N x 3
            else:
        ...
        if cfg.predict_type == "angles":
            ...
            joints_pred = {}
            if cfg.predict_2p5d:
                loss["joint_2p5d_hm"] = self.joint_2p5d_loss(
                    joint_2p5d_hm, targets["joint_coord"], meta_info["joint_valid"]
                )
                ...
                ) = self.joint_vecs_loss(
                    joint_vecs,
                    targets["joint_cam_no_trans"],
                    meta_info["joint_valid"],
                    targets["joint_coord"][:, :, :2],
                    cam_param,
                    rel_trans,
                )
                out["joint_3d_right_out"] = joints_pred["joints_right"]
                out["joint_3d_left_out"] = joints_pred["joints_left"]
                joints_pred["joints2d_right"] = joints_pred["joints_right"][:, :, :, :2] * cam_param[
                    :, :, :1
                ].unsqueeze(2) + cam_param[:, :, 1:].unsqueeze(2)
        ...
        loss["joint_heatmap"] = self.joint_heatmap_loss(
            joint_heatmap_out, target_joint_heatmap, meta_info["joint_valid"], meta_info["hm_valid"]
        )
        ...
        if cfg.enc_layers > 0:
            loss["cls"], row_inds_batch_list, asso_inds_batch_list = self.joint_class_loss(
                joint_loc_pred_np,
                targets["joint_coord"][:, :, :2],
                mask_np,
                joint_class.permute(0, 2, 1, 3),
                meta_info["joint_valid"],
                peak_joints_map_batch,
                targets["joint_cam_no_trans"],
        ...
        if cfg.use_bottleneck_hand_type:
            loss["hand_type"] = self.bottleneck_hand_type_loss(
                bottleneck_hand_type, targets["hand_type"], meta_info["hand_type_valid"]
            )
        else:
            if cfg.hand_type == "both":
        ...
        if cfg.predict_type == "angles":
            ...
            loss["joints_loss"], loss["joints2d_loss"] = self.joints_loss(
                rel_trans,
                targets["joint_cam_no_trans"],
                meta_info["joint_valid"],
                targets["joint_coord"][:, :, :2],
            ...
            # loss["vertex_loss"] = torch.zeros((1,)).to(pose.device) # TODO: get this right
            loss["vertex_loss"] = self.vertex_loss(targets["verts"], meta_info["mano_valid"], joints_pred)

        if cfg.hand_type == "both":
            loss["rel_trans"] = self.rel_trans_loss(rel_trans, targets["rel_trans_hands_rTol"], meta_info["root_valid"])

        if cfg.use_2D_loss and ((not cfg.predict_2p5d) or (cfg.predict_type == "angles")):
            loss["cam_scale"], loss["cam_trans"], cam_param_gt = self.cam_param_loss(
                targets["joint_cam_no_trans"],
                meta_info["joint_valid"],
                targets["joint_coord"][:, :, :2],
                cam_param,

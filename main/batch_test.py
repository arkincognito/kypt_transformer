...
def run_batch_test():
    ...
    for path in glob.glob(ckpt_path_str):
        ...
        for key in loss_cum.keys():
            ...
            loss_cum[key] = loss_cum[key] / cfg.training_batch_length

        # dump the losses
        if cfg.predict_type == "angles":
            summary_writer.add_scalar("pose loss", loss["pose"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("shape_reg loss", loss["shape_reg"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("vertex loss", loss["vertex_loss"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("shape", loss["shape_loss"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("joints loss", loss["joints_loss"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("joints2d_loss loss", loss["joints2d_loss"], epoch * cfg.training_batch_length + itr)

        elif cfg.predict_type == "vectors":
            if cfg.predict_2p5d:
                summary_writer.add_scalar("joint_2p5d_hm loss", loss["joint_2p5d_hm"], epoch * cfg.training_batch_length + itr)
            else:
                summary_writer.add_scalar("joint_vec loss", loss["joint_vec"], epoch * cfg.training_batch_length + itr)
                summary_writer.add_scalar("joints loss", loss["joints_loss"], epoch * cfg.training_batch_length + itr)
                summary_writer.add_scalar("joints2d_loss loss", loss["joints2d_loss"], epoch * cfg.training_batch_length + itr)

        if cfg.hand_type == "both":
            summary_writer.add_scalar("rel_trans loss", loss["rel_trans"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("hand_type loss", loss["hand_type"], epoch * cfg.training_batch_length + itr)

        if cfg.has_object:
            summary_writer.add_scalar("obj_seg loss", loss["obj_seg"], epoch * cfg.training_batch_length + itr)

            summary_writer.add_scalar("obj_rot loss", loss["obj_rot"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("obj_trans loss", loss["obj_trans"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("obj_corners loss", loss["obj_corners"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("obj_corners_proj loss", loss["obj_corners_proj"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("obj_weak_proj loss", loss["obj_weak_proj"], epoch * cfg.training_batch_length + itr)

        summary_writer.add_scalar("cls loss", loss["cls"], epoch * cfg.training_batch_length + itr)

        summary_writer.add_scalar("heatmap loss", loss["joint_heatmap"], epoch * cfg.training_batch_length + itr)

        summary_writer.add_scalar(
            "Learning rate", tester.lr_scheduler.get_last_lr()[-1], epoch * cfg.training_batch_length + itr
        )
        summary_writer.add_scalar("total loss", sum(loss[k] for k in loss), epoch * cfg.training_batch_length + itr)

        if cfg.use_2D_loss and ((not cfg.predict_2p5d) or (cfg.predict_type == "angles")):
            summary_writer.add_scalar("cam trans", loss["cam_trans"], epoch * cfg.training_batch_length + itr)
            summary_writer.add_scalar("cam scale", loss["cam_scale"], epoch * cfg.training_batch_length + itr)
